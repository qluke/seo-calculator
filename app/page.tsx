...
export default function SeoCalculator() {
  ...
  const calculateResults = () => {
    ...
    const roi = totalLinkCost > 0 ? Number((yearlyIncome / totalLinkCost).toFixed(2)) : 0
  ...
  // 输入控件组件
  const InputControl = ({ label, value, onChange, min, max, step = 1, info }) => (
    <div className="mb-3">
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 max-w-2xl mx-auto text-sm">
            SEO 收入目标计算器根据目标收入和参数分解目标，计算实现目标所需的条件，帮助您更好地评估 SEO 投资和回报。
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              <CardContent>
                <div className="space-y-1">
                  <InputControl
                    ...
                    step={100}
                  />
                  <InputControl label="网站数量" value={websiteCount} onChange={setWebsiteCount} min={1} max={10} />
                  <InputControl
                    label="每个网站的关键词数量"
            ...
            <Card>
              ...
              <CardContent className="text-sm text-slate-600">
                <ol className="list-decimal list-inside space-y-1.5">
                  <li>
                    月收入 ${monthlyIncome} 除以 30 天 = 日目标 ${results.dailyIncome}
                  </li>
                  <li>
                    每位访客产生 ${results.adRevenuePerUv} 广告收入 + ${results.saasRevenuePerUv} SaaS收入 = $
                    {results.revenuePerUv}
                  </li>
                  <li>
                    实现日收入 ${results.dailyIncome}，所需总访客数: {results.totalDailyUv}
                  </li>
                  <li>
                    分布在 {websiteCount} 个网站上，每个网站所需访客: {results.uvPerSite}
                  </li>
                  <li>
                    每个网站目标 {keywordsPerSite} 个关键词，排名位置 {rankPosition} 的点击率为{" "}
                    {rankClickRates[rankPosition]}%，每个关键词所需日搜索量: {results.searchVolumePerKeyword}
                  </li>
                  <li>
                    每位访客产生 {pvPerUv} 次页面浏览，预期页面浏览量: {results.pvPerSite}
                  </li>
                  <li>
                    基于关键词难度 KD {keywordKD}，需要 {results.requiredDomains} 个反向链接域名
                  </li>
                  <li>预计链接建设成本 ${results.totalLinkCost} (建议预算不超过6个月收入)</li>
                </ol>
              </CardContent>
            </Card>
          </div>
          ...
          <div className="lg:col-span-7 space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <Card className="transition-all hover:shadow-md">
                <CardHeader className="pb-2 pt-4">
                  <CardDescription className="text-xs">日收入目标</CardDescription>
                  <CardTitle className="text-xl text-blue-600">${results.dailyIncome}</CardTitle>
                </CardHeader>
              </Card>
              <Card className="transition-all hover:shadow-md">
                <CardHeader className="pb-2 pt-4">
                  <CardDescription className="text-xs">每位访客收入</CardDescription>
                  <CardTitle className="text-xl text-blue-600">${results.revenuePerUv}</CardTitle>
                </CardHeader>
              </Card>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              <Card className="transition-all hover:shadow-md">
                <CardHeader className="pb-2 pt-4">
                  <CardDescription className="text-xs">每个网站所需日访客</CardDescription>
                  <CardTitle className="text-xl text-blue-600">{results.uvPerSite}</CardTitle>
                </CardHeader>
              </Card>
              <Card className="transition-all hover:shadow-md">
                <CardHeader className="pb-2 pt-4">
                  <CardDescription className="text-xs">每个网站预期日页面浏览</CardDescription>
                  <CardTitle className="text-xl text-blue-600">{results.pvPerSite}</CardTitle>
                </CardHeader>
              </Card>
              <Card className="transition-all hover:shadow-md">
                <CardHeader className="pb-2 pt-4">
                  <CardDescription className="text-xs">每个关键词所需日搜索量</CardDescription>
                  <CardTitle className="text-xl text-blue-600">{results.searchVolumePerKeyword}</CardTitle>
                </CardHeader>
              </Card>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <Card className="transition-all hover:shadow-md">
                <CardHeader className="pb-2 pt-4">
                  <CardDescription className="text-xs">每个网站所需反向链接域名</CardDescription>
                  <CardTitle className="text-xl text-blue-600">{results.requiredDomains}</CardTitle>
                </CardHeader>
              </Card>
              <Card
                className={`transition-all hover:shadow-md ${results.totalLinkCost > monthlyIncome * 6 ? "bg-red-50 border-red-200" : ""}`}
              >
                <CardHeader className="pb-2 pt-4">
                  <CardDescription className="text-xs">预计总链接建设成本</CardDescription>
                  <CardTitle
                    className={`text-xl ${results.totalLinkCost > monthlyIncome * 6 ? "text-red-600" : "text-blue-600"}`}
                  >
            ...
            <div className="grid grid-cols-2 gap-4">
              <Card className="transition-all hover:shadow-md">
                <CardHeader className="pb-2 pt-4">
                  <CardDescription className="text-xs">12个月预计收入</CardDescription>
                  <CardTitle className="text-xl text-blue-600">${results.yearlyIncome}</CardTitle>
                </CardHeader>
              </Card>
              <Card className={`transition-all hover:shadow-md ${results.roi < 1 ? "bg-red-50 border-red-200" : ""}`}>
                <CardHeader className="pb-2 pt-4">
                  <CardDescription className="text-xs">ROI (12个月收入/链接成本)</CardDescription>
                  <CardTitle className={`text-xl ${results.roi < 1 ? "text-red-600" : "text-blue-600"}`}>
                    {results.roi}x
                  </CardTitle>
                </CardHeader>
              </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-4 text-sm">
                <p>
                  ...
                  ， 通过 <span className="text-blue-600 font-medium bg-blue-50 px-1 rounded">{websiteCount}</span>{" "}
                  个网站， 每个网站目标{" "}
                  <span className="text-blue-600 font-medium bg-blue-50 px-1 rounded">{keywordsPerSite}</span>{" "}
                  个关键词。 日收入目标为{" "}
                  <span className="text-blue-600 font-medium bg-blue-50 px-1 rounded">${results.dailyIncome}</span>。
                </p>

                <p>
                  基于当前设置，每位访客价值包括： 广告收入{" "}
                  <span className="text-blue-600 font-medium bg-blue-50 px-1 rounded">${results.adRevenuePerUv}</span> +
                  SaaS收入{" "}
                  <span className="text-blue-600 font-medium bg-blue-50 px-1 rounded">${results.saasRevenuePerUv}</span>{" "}
                  = 总价值{" "}
                  <span className="text-blue-600 font-medium bg-blue-50 px-1 rounded">${results.revenuePerUv}</span>。
                  每位访客平均浏览 <span className="text-blue-600 font-medium bg-blue-50 px-1 rounded">{pvPerUv}</span>{" "}
                  个页面。
                </p>
                    ...
                    <ul className="space-y-1.5 pl-5 list-disc text-slate-700">
                      {results.risks.inefficientKD && (
                        <li>
                          ...
                          {formatNumber(results.searchVolumePerKeyword)}) 较低，导致ROI不佳。建议：
                      ...
                      {results.risks.highSearchVolume && (
                        <li>
                          当前所需搜索量 ({formatNumber(results.searchVolumePerKeyword)})
                          过高，考虑增加网站数量、增加关键词数量或调整收入预期
                        </li>
                      )}
                      {results.risks.lowRoi && <li>当前ROI ({results.roi}x) 较低，考虑提高用户价值或降低获取成本</li>}
                      {results.risks.highCost && (
                        <li>链接建设成本 (${formatNumber(results.totalLinkCost)}) 过高，考虑目标难度更低的关键词</li>
                      )}
                    </ul>
                  </div>
                )}
        ...
        <div className="mt-8 pt-6 border-t border-slate-200 text-center">
          ...
          <div className="text-slate-400 text-xs">
            <p>
              © 2025 SEO收入目标计算器 <span className="mx-2 text-slate-300">|</span> 保留所有权利
            </p>
          </div>
        </div>
      </div>
